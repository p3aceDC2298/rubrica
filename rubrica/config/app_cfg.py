...
from tgext.pluggable import plug, replace_template, plug_url
...
base_config.sa_auth.cookie_secret = "d0118461-89ba-41a2-b4fd-0725b9957eb1"
...
class ApplicationAuthMetadata(TGAuthMetadata):
    ...
    def authenticate(self, environ, identity):
        ...
        user = self.sa_auth.dbsession.query(self.sa_auth.user_class).filter(
                or_(self.sa_auth.user_class.user_name == login,
                    self.sa_auth.user_class.email_address == login)
            ).first()
    ...
    def get_user(self, identity, userid):
        return self.sa_auth.dbsession.query(self.sa_auth.user_class).filter(
                ...
                self.sa_auth.user_class.email_address==userid)
            ).first()
...
#    'rubrica.templates.registration')
# Non riesco a passare il valore dei miei form al submit
# usando dei miei form, l'unica cosa che mi viene in mente
# per far funzionare il tutto , è di riscrivermi la funzione from scratch,
# ma cosi non avrebbe piu senso usare il pluggabile
